...
export default function WhatsAppBoostView({ 
  ...
  // Navbar එකේ වගේම Live Balance එක handle කරන්න local state එකක්
  ...
  // මුලින්ම load වෙනකොට සහ user මාරු වෙනකොට balance එක ගන්න
  useEffect(() => {
    syncBalance();
    // Navbar එකේ වගේම හැම තත්පර 30කටම සැරයක් background එකේ check කරනවා
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (deductRes.ok && deductData.success) {
        // 2. සල්ලි කැපුන ගමන් වහාම local balance එක update කරනවා
        await syncBalance(); 

        // 3. දැන් බොට් එකට signal එක යවනවා
        ...
        if (botRes.ok && botData.success) {
          ...
          setLink('');
          // අන්තිමට Global states (History etc) refresh කරනවා
  ...
  return (
    // Header එක වැහෙන්නේ නැති වෙන්න pt-24 (top padding) එකක් දැම්මා
    <div className="animate-fade-in pt-24 pb-10 px-4">
      <div className="max-w-md mx-auto bg-white dark:bg-[#0f172a]/60 rounded-[2.5rem] border border-slate-200 dark:border-white/5 overflow-hidden shadow-2xl backdrop-blur-md">
        ...
        <div className="p-8 space-y-6">
          ...
          <p className="text-center text-[7px] font-bold text-slate-400 uppercase tracking-widest opacity-50 pb-2">
